...
class SupplierController extends BaseController
{
    ...
    {
        if (!$this->manager->can('supplier/list')) {
    ...
    public function actionStatus()
    {
        ...
        ManagerLog::info($this->manager->id, '保存供货商【' . $supplier->id . '】状态【' . $supplier->status . '】。');
    ...
    {
        if (!$this->manager->can('supplier/edit')) {
            throw new ForbiddenHttpException('没有权限。');
        }
        ...
        if ($supplier->load($this->post()) && $configForm->load($this->post())) {
            ...
            if ($supplier->save()) {
                ...
                ManagerLog::info($this->manager->id, '保存供货商', print_r($supplier->attributes, true));
                Yii::$app->session->addFlash('success', '数据已保存。');
                Yii::$app->session->setFlash('redirect', json_encode([
                    'url' => Url::to(['/admin/supplier/list']),
                    'txt' => '供货商列表'
                ]));
            }
        }
    ...
    {
        if (!$this->manager->can('supplier/goods-list')) {
            throw new ForbiddenHttpException('没有权限。');
        }
    ...
    {
        if (!$this->manager->can('supplier/goods-edit')) {
            throw new ForbiddenHttpException('没有权限。');
        }
        ...
        if ($this->isPost()) {
            ...
            ManagerLog::info($this->manager->id, '保存商品【' . $goods->id . '】结算价【' . $goods->supplier_price . '】。');
            ...
            if (!empty($skuSupplierPrice) && is_array($skuSupplierPrice)) {
                foreach ($skuSupplierPrice as $skuId => $supplierPrice) {
                    ...
                    if ($sku->save(false)) {
                        ManagerLog::info($this->manager->id, '保存商品【' . $goods->id . '】规格【' . $sku->key_name . '】结算价【' . $sku->supplier_price . '】。');
                    }
                }
            }
            Yii::$app->session->addFlash('success', '数据已保存。');
            Yii::$app->session->setFlash('redirect', json_encode([
                'url' => Url::to(['/admin/supplier/goods-list']),
                'txt' => '商品列表'
            ]));
        }
    ...
    {
        if (!$this->manager->can('supplier/order-list')) {
            throw new ForbiddenHttpException('没有权限。');
        }
