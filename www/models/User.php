...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['gender', 'hand_count'], 'default', 'value' => 0],
            ['gender', 'in', 'range' => [User::GENDER_MALE, User::GENDER_FEMALE, User::GENDER_UNKNOWN, User::GENDER_SECRET]],
            [['team_pid', 'level_id', 'prepare_count', 'real_name', 'wx_no', 'pid', 'shop_name','wx_nick_name', 'address', 'create_time'], 'safe'],
        ];
    }
...
//            }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'level_id' => '等级',
            'create_time' => '注册时间',
            'commission_ratio_1' => '1级比例',
            'commission_ratio_2' => '2级比例',
            'commission_ratio_3' => '3级比例',
        ];
    }
    ...
    public static function findByToken($token)
    {
        try {
            ...
            return User::findOne(['id' => $payloadJson['uid']]);
        } catch (Exception $e) {
...
//            }
    ...
    {
        //如果上级都没激活以上所有都不给补贴
        if (empty($user->parent) || $user->parent->status == User::STATUS_WAIT) {
            return true;
        }
        //先把三个上级都找出来  看看是否需要  给补贴
        //给补贴 则往 user_subsidy 插入记录  并且更新每个上级的 销售补贴总额
        ...
        $is_three = 0;
        //直接2级
        if (empty($user->parent->parent) || $user->parent->parent->status == User::STATUS_WAIT) {
            return true;
        }
        if ($user->parent && $user->parent->parent) {
            $p2 = $user->parent->parent;
        ...
        //直接3级  只有 第二级  是店主 或者 服务商的时候  才考虑要不要给 育成了店主服务商的  店主或者服务商 补贴
        if ($is_three == 1) {
            if (empty($user->parent->parent->parent) || $user->parent->parent->parent->status == User::STATUS_WAIT) {
                return true;
            }
            if ($user->parent && $user->parent->parent && $user->parent->parent->parent) {
                $p2 = $user->parent->parent;
...
//        return $tree;
    ...
    public function getComputeCommission()
    {
        ...
        foreach ($queryOrder->each() as $model) {
            ...
            if ($model->user->status == User::STATUS_OK && $this->status == User::STATUS_WAIT) {
                continue;
            }
            /** @var OrderItem $item */
    ...
    public function addScore($score,$code='',$remark='')
    {
        $trans = Yii::$app->db->beginTransaction();
        try {
            $r = UserAccount::updateAllCounters(['score' => $score], ['uid' => $this->id]);
            if ($r <=0) {
                throw new Exception('更新积分失败。');
            }
    ...
    public function updateCoupons($gid,$coupon_id)
    {
        ...
        try {
            ...
            $r= GoodsCouponGiftUser::find()
                ->where(['or', ['status' => GoodsCouponGiftUser::STATUS_WAIT], ['status' => GoodsCouponGiftUser::STATUS_LOCK]])
            ...
            if ($r <= 0 ) {
                ...
                for ($i = 0; $i < $coupon->count; $i++) {
                    $data[] = [
                        ...
                        'uid' => $this->id,
                        'create_time' => time(),
                        'status' => GoodsCouponGiftUser::STATUS_WAIT,
                    ];
                }
                //再执行批量插入
                if (isset($data))
                {
                    Yii::$app->db->createCommand()
                        ->batchInsert(GoodsCouponGiftUser::tableName(),['cid','gid','uid','create_time','status'],
                            $data)
                        ->execute();
                }
                ...
                /** @var $user_coupon_lock GoodsCouponGiftUser */
                if(empty($coupon_id))
                {
                    throw new Exception('订单优惠券参数错误');
                }
                $user_coupon_lock = GoodsCouponGiftUser::findOne($coupon_id);
                if ($user_coupon_lock->status != GoodsCouponGiftUser::STATUS_LOCK) {
                    throw new Exception('未找到可更新优惠券');
                }
                $user_coupon_lock->status = GoodsCouponGiftUser::STATUS_USED;
     ...
     * @return  mixed | string
     */
    public function getBottomUsers($uid, $uidArr=''){
        $userList = User::find()->select('id')->where(['pid' => $uid])->all();
        foreach ($userList as $key=>$value){
            $uidArr .= $value['id'].',';
            $user = User::find()->select('id')->where(['pid' => $value['id']])->all();
            if($user){
                $uidArr = $this->getBottomUsers($value['id'],$uidArr);
            }
        }
        return $uidArr;
    }
     ...
     * @return  mixed | string
     */
    public function getOkBottomUsers($uid, $uidArr=''){
        $userList = User::find()->select('id')->where(['pid' => $uid, 'status' => User::STATUS_OK])->all();
        foreach ($userList as $key=>$value){
            $uidArr .= $value['id'].',';
            $user = User::find()->select('id')->where(['pid' => $value['id'], 'status' => User::STATUS_OK])->all();
            if($user){
                $uidArr = $this->getOkBottomUsers($value['id'],$uidArr);
            }
        }
        return $uidArr;
    }
     ...
     * @return  mixed | string
     */
    public function getOkBottomUser($uid, $uidArr='', $time = ''){
        if (empty($time)) {
            $userList = User::find()->select('id')->where(['pid' => $uid, 'status' => User::STATUS_OK])->all();
            foreach ($userList as $key=>$value){
                $uidArr .= $value['id'].',';
                $user = User::find()->select('id')->where(['pid' => $value['id'], 'status' => User::STATUS_OK])->all();
                if($user){
                    $uidArr = $this->getOkBottomUser($value['id'],$uidArr, $time);
                }
            }
        } else {
            $userList = User::find()->select('id')->where(['pid' => $uid, 'status' => User::STATUS_OK])
                ->andWhere(['>=', 'handle_time', $time])
                ->all();
            foreach ($userList as $key=>$value){
                $uidArr .= $value['id'].',';
                $user = User::find()->select('id')->where(['pid' => $value['id'], 'status' => User::STATUS_OK])
