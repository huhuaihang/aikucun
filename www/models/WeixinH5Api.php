 ...
 * @package app\models
 */
class WeixinH5Api extends Model
{
    /**
     * @var string 接口域名
     */
    ...
    {
        $this->app_id = System::getConfig('weixin_h5_app_id');
        $this->mch_id = System::getConfig('weixin_h5_mch_id');
        $this->api_key = System::getConfig('weixin_h5_api_key');
        $this->cert_file = System::getConfig('weixin_h5_cert_file');
        $this->key_file = System::getConfig('weixin_h5_key_file');
        $this->notify_url = System::getConfig('weixin_h5_notify_url');
        parent::init();
    }
     ...
     * @param array $scene_info 场景信息（区分浏览器、WebView），用于拉起微信
     * @return string mweb_url：跳转地址（只有MWEB支付方式才有）
     * @throws Exception
     */
    public function unifiedOrder($name, $no, $money, $trade_type = 'MWEB', $scene_info = [])
    {
        ...
        $post['spbill_create_ip'] = Util::realIp();
        $post['notify_url'] = $this->notify_url;
        $post['trade_type'] = $trade_type;
        $post['scene_info'] = json_encode($scene_info, JSON_UNESCAPED_UNICODE);
