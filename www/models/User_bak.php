 ...
 */
class User_bak extends ActiveRecord implements IdentityInterface
{
    const GENDER_MALE = 1;
...
//                    ->orWhere(['between', 'id', 3660, 3720])
    ...
    public function tree($user) {
        if ($user->parent && ($user->parent->level_id == 2 || $user->parent->level_id == 3)) {
            return $user->parent;
        } else {
            if (!$user->parent) {
                return $user;
...
//        return $tree;
    ...
    {
        if ($user->teamParents && (($user->teamParents->level_id == 2 || $user->teamParents->level_id == 3) || is_null($user->teamParents->pid))) {
            return $user->teamParents;
        } else {
...
//        return false;
    ...
    public function getChildBuyRatio()
    {
        ...
        Yii::warning($this->id . '+++'. $this->level_id. '_++'.$ratio);
        return $ratio;
    }
