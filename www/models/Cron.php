 ...
 *
 *            This is a simple script to parse cron syntax to get the execution time
 *
 *            Eg.: $timestamp = Cron::parse('12 * * * 1-5');
 *
 *
...
{
    /**
     * Finds next execution time(stamp) from cron syntax,
     * after given starting timestamp (or current time if emitted)
     *
    ...
    public static function parse($_cron_string, $_after_timestamp = null)
    {
        ...
        $date = array(
            'minutes' => Cron::_parseCronNumbers($cron[0], 0, 59),
            'hours' => Cron::_parseCronNumbers($cron[1], 0, 23),
            'dom' => Cron::_parseCronNumbers($cron[2], 1, 31),
            'month' => Cron::_parseCronNumbers($cron[3], 1, 12),
            'dow' => Cron::_parseCronNumbers($cron[4], 0, 6)
        );
