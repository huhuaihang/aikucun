...
//        }
        if (is_uploaded_file($_FILES['file']['tmp_name'])) {
            ...
            if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
                ...
                $object_vend_banner = "goods_trace/" . $this->fileName;            //拼接存储路径和文件名称，上传第一个参数

                $oss = new AliyunOssApi();

                $this->vend_banner_url = $oss->uploadFile($object_vend_banner, $this->filePath);
//                $this->vend_banner_url=Yii::$app->params['site_host'].'/uploads/'.$object_vend_banner;
    ...
    public function getFileInfo()
    {
        return array(
           ...
           // "url"      =>  Yii::$app->params['site_host'].'/'.$this->fullName,
            'path'=>$this->fullName,
             'url'=>$this->vend_banner_url,
            "title"    => $this->fileName,
