 ...
 * </script>
 */
class FileUploadWidgetOld extends InputWidget
{
    /**
     * @var string 文件上传路径
     */
    ...
    public function run()
    {
        ...
        $id = rand(0, 99999999);
        $input = '<input id="fileupload_' . $id . '" type="file" name="files[]" data-url="' . $this->url . '"' . ($this->multiple ? 'multiple' : '') . ' />';
...
JS
        ...
        if (!empty($this->click_node)) {
            ...
            $this->getView()->registerJs(
<<<JS
$('{$this->click_node}').click(function() { $('#fileupload_{$id}').click() });
JS
            );
        }
