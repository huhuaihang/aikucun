$(function(){
    ...
    $('.edit_order').click(function(){
        $(this).parent('.shop-info-text').find('.edit_delate').show();
    });
    //单品删除
    $('.shanchu').click(function(){
        var id = $(this).data('id');
        if(!id){
            layer.msg('操作错误。', function () {});
            return false;
        }
        deleteCart(id);
        var goods_item_count = $(this).parents('li').parents('.goods_item').children('li').length;
        if(goods_item_count - 1 ==0){
            $(this).parents('li').parents('.goods_item').parents('.shop-group-item').remove();
        }else{
	...
	$(".minus").click(function() {
        ...
        setAmount($(this).data('id'), parseInt(t.text()));
  ...
  function deleteCart(id){
      $.getJSON(AJAXURL['deleteCart'], {'id': id}, function (json) {
          if (callback(json)) {
  ...
  function setAmount(id, amount){
      $.getJSON(AJAXURL['setAmount'], {'id': id, 'amount': amount}, function (json) {
          if (callback(json)) {
          }
      });
  }
