        ...
        _create:function() {
            ...
            (function() {
                if(me.data('itemRender')) {
                    ...
                    while(j = content[i++]) k.push('<div class="ui-slider-item"><a href="' + j.href + '"><img lazyload="' + j.pic + '"/></a>' + (j.title ? '<h4>' + j.title + '</h4>': '') +(j.descr ? '<p>' + j.descr + '</p>': '') +(j.price ? '<p class="price">' + j.price + '</p>': '') + (j.price ? '<span>' + j.text + '</span>': '')+'</div>');
                }
