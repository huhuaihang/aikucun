...
<br>
<div class="form-group">
    <a href="<?php echo Url::to(['/admin/user/master-edit']);?>" class="btn btn-info btn-sm">添加</a>
...
<!--            <td>--><?php //echo (empty($model->userLevel)) ? '待激活': Html::encode($model->userLevel->name).'<br> (等级金额)'.$model->subsidy_money;?><!--</td>-->
...
//                    ['icon' => 'fa fa-users', 'href' => Url::to(['/admin/user/relation-view', 'id' => $model->id]), 'target' =>'_blank', 'btn_class' => 'btn btn-xs', 'tip' => '团队图谱','alt' => '团队图谱'],
                    ['icon' => 'fa fa-bars', 'href' => Url::to(['/admin/user/master-account-list', 'uid' => $model->id]), 'target' =>'_blank', 'btn_class' => 'btn btn-xs', 'tip' => '结算单列表记录'],
//                    ['icon' => 'fa fa-bars', 'href' => Url::to(['/admin/user/growth-list', 'uid' => $model->id]), 'target' =>'_blank', 'btn_class' => 'btn btn-xs', 'tip' => '成长值记录'],
                    ['icon' => 'fa fa-btc', 'href' => Url::to(['/admin/user/master-account-log-edit', 'uid' => $model->id]), 'target' =>'_blank', 'btn_class' => 'btn btn-xs', 'tip' => '增加结算单'],
...
//                    ['icon' => 'fa fa-tags', 'onclick' => 'bindCard(' . $model->id . ')', 'btn_class' => 'btn btn-xs btn-warning', 'tip' => '绑定会员卡'],
                    ['icon' => 'fa fa-pencil', 'href' => Url::to(['/admin/user/master-edit', 'id' => $model->id]), 'btn_class' => 'btn btn-xs btn-success', 'tip' => '编辑', 'color' => 'green'],
...
//    }
    ...
    function deleteUser(id) {
        ...
        $.getJSON('<?php echo Url::to(['/admin/user/master-delete']);?>', {'id':id}, function(json) {
            if (callback(json)) {
                $('#data_' + id).remove();
            }
        });
