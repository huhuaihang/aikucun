...
<!--    <label class="control-label" for="score">发放说明</label>-->
<!--    <input id="name" class="form-control" name="name" aria-required="true">-->
    <label class="control-label" for="to_mobile">接收人员手机号码</label>
    <input  class="form-control tagsinput"  id="tagsinputval" data-role="tagsinput" name="to_mobile" aria-required="true" style="min-width: 200px;" />
...
<script>
    ...
    function send() {
        ...
        $.post('<?php echo Url::to(['/admin/user/send-pack-coupon']);?>', {'data':form_data}, function (json) {
               if(json['message'])
               {
                   ...
                   $('.btn-primary').removeAttr("disabled"); //设置变灰按钮
                    ...
                    if(json['error_mobile'].length > 0)
                    {
                        ...
                        json['error_mobile'].forEach(function (item) {
                            str+='<p style="color: red">'+'手机号'+item['mobile']+'卡券发放失败['+item['message']+']'+'</p>';
                        })
                        $('.help-block').html(str);
                        ...
                        setTimeout(function(){  window.location.href='/admin/user/pack-coupon-list'; }, 1000);
                    }
