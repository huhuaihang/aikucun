...
<div class="box" id="app">
    ...
    <header class="mall-header">
        ...
        <div class="mall-header-title">个人商家入驻申请</div>
    ...
    <div class="container">
        <!--申请-->
            <div class="b_edit_add">
                <input type="hidden" v-model="MerchantPersonForm.area" name="MerchantPersonForm.area">
                <label class="ubb b_magt1">
                    <span>店铺名称</span>
                    <input type="text" id="merchantjoinform-shop_name" v-model="MerchantPersonForm.shop_name" name="shop_name" placeholder="请填写店铺名称">
                </label>
                <label class="ubb">
                    <span>联系人姓名</span>
                    <input type="text" id="merchantjoinform-contact_name" v-model="MerchantPersonForm.contact_name" name="contact_name" placeholder="请填写真实姓名">
                </label>
                <label class="ubb">
                    <span>联系电话</span>
                    <input type="text" id="merchantjoinform-mobile" v-model="MerchantPersonForm.mobile" name="mobile" placeholder="请填写联系电话">
                    <a class="b_get_yzm" @click="sendSmsCode">{{ btn_txt }}</a>
                </label>
                <label class="ubb">
                    <span>短信验证码</span>
                    <input type="text" id="merchantjoinform-sms_code" v-model="MerchantPersonForm.sms_code" name="sms_code" placeholder="请填写收到的验证码">
                </label>
                <label class="ubb b_magt1">
                    <span>登录邮箱</span>
                    <input type="text" id="merchantjoinform-username" v-model="MerchantPersonForm.username" name="username" placeholder="请填写商户登录邮箱（登录用）">
                </label>
                <label class="ubb">
                    <span>商户密码</span>
                    <input type="text" id="merchantjoinform-password" v-model="MerchantPersonForm.password" name="password" placeholder="请填写商户密码（登录用）">
                </label>
                <label class="ubb">
                    <span>代理商</span>
                    <input type="text" id="merchantjoinform-agent_username" v-model="MerchantPersonForm.agent_username" name="agent_username" placeholder="请填写代理商登录邮箱（非必填项）">
                </label>
        ...
        methods: {
            uploadIdCardFront: function (e) {
                ...
                apiFile('<?php echo Url::to(['/api/default/upload', 'dir' => 'merchant']);?>', e.target.files[0], function (json) {
                    if (callback(json)) {
                        var url = json['url'];
                        self.MerchantPersonForm.id_card_front = json['uri'];
            ...
            uploadIdCardBack: function (e) {
                ...
                apiFile('<?php echo Url::to(['/api/default/upload', 'dir' => 'merchant']);?>', e.target.files[0], function (json) {
                    if (callback(json)) {
                        var url = json['url'];
                        self.MerchantPersonForm.id_card_back = json['uri'];
            ...
            sendSmsCode: function () {
                ...
                if (!/^\d{11}$/.test(this.MerchantPersonForm.mobile)) {
                    layer.msg('手机号码格式错误。', function () {});
                    app.MerchantPersonForm.mobile.focus();
                    return false;
                }
                apiGet('<?php echo Url::to(['/api/join/send-sms-code-merchant']);?>', {'mobile':this.MerchantPersonForm.mobile}, function (json) {
                    if (callback(json)) {
            ...
            submit: function () {
                if (!/^.{1,32}$/.test(this.MerchantPersonForm.username)) {
                    layer.msg('用户名格式错误。', function () {});
                    app.$refs.username.focus();
                    return false;
                }
                if (!/^.+$/.test(this.MerchantPersonForm.password)) {
                ...
                if (!/^\d{4}$/.test(this.MerchantPersonForm.sms_code)) {
                    layer.msg('手机验证码格式错误，只能填写4位数字。', function () {});
                    app.$refs.code.focus();
                    return false;
                }
                this.MerchantPersonForm.cid_list = [this.cid1].concat(this.cid_list2);
                apiPost('<?php echo Url::to(['/api/join/save-merchant']);?>', this.MerchantPersonForm, function (json) {
                    if (callback(json)) {
            ...
            loadCity: function () {
                $('[name="MerchantPersonForm.area"]').after(
                    '<div class="ubb">' +
                    '<span>所在地区</span>' +
                ...
                $('#citys').citys({
                    ...
                    code: this.MerchantPersonForm.area,
                    required: true,
                    onChange: function (city) {
                        app.MerchantPersonForm.area = city['code'];
                    }
                });
            },
